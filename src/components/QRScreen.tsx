...
export default function QRScreen({ status, qrPng, updatedAt }: QRScreenProps) {
	...
	return (
		<div className="glass-panel rounded-3xl p-8 max-w-md w-full mx-auto text-center shadow-2xl relative border-outline-variant/20 bg-surface/80 backdrop-blur-xl">
			<div className="mb-6 size- rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center mx-auto shrink-0 glow-active">
			...
			<div className="relative size- bg-[#f1ead7] border border-primary/30 rounded-2xl flex flex-col items-center justify-center overflow-hidden mb-6 p-4 mx-auto shadow-inner">
				{status === "qr" && qrPng ? (
					...
					<div className="flex flex-col items-center gap-3">
						<div className="size- border-2 border-primary border-t-transparent rounded-full animate-spin glow-active"></div>
			...
			<div className="flex items-center justify-center gap-2.5 bg-surface-container-low/40 px-4 py-2 rounded-xl border border-outline-variant/10 text-[10px] mb-4 w-fit mx-auto">
				{status === "qr" && (
					<>
						<span className="relative flex size-">
							<span className="animate-ping absolute inline-flex size- rounded-full bg-secondary opacity-75"></span>
							<span className="relative inline-flex rounded-full size- bg-secondary"></span>
						</span>
				...
				{status === "disconnected" && (
					<>
						<span className="relative flex size-">
							<span className="relative inline-flex rounded-full size- bg-on-surface-variant/40"></span>
						</span>
